...
wh_default_model_names = (
    "arviz_fit_null_hierarchical_model.pkl",
...
cue_default_model_names = (
    "arviz_fit_null_precue_model.pkl",
    "arviz_fit_spatial_error_precue_model.pkl",
    "arviz_fit_hybrid_error_precue_model.pkl",
)
cue_default_model_keys = ("lin", "spatial", "hybrid")

model_folder_template = (
    "swap_errors/neural_model_fits/{num_cols}_colors/"
...
lm_template = (
    "fit_(nulls_)?lmtc_(rt[0-9.]+_)?(?P<trial_type>pro|retro)_[a-z0-9_]*"
...
lm_targ_template = (
    "fit_nulls_lmtc_{trl_type}_no_{region}"
    "_{timing}"
    "\\-presentation_"
...
def make_lm_lists(*args, **kwargs):
    ...
    for r in regions:
        for t in times:
            s = list_lm_runinds(*args, **kwargs, region=r, timing=t, trl_type=use_tt[t])
...
def load_lm_results(
    runinds, folder="swap_errors/lms/", templ=lm_template, swap_mean=True
):
    jobids = "|".join(runinds)
    ...
    for fl in fls:
        ...
        if m is not None:
            ...
            tt_timing_dict[monkey] = ((null_col, swap_col), (null_cue, swap_cue), xs)
...
def print_error_details(fit, Rhat=True, eps=0.05, divergence=True, **kwargs):
    if not Rhat:
        rh = az.rhat(fit)
        eps = 0.05
        for k in rh.keys():
...
def load_o_fits(
    ...
    period="WHEEL_ON",
    load_data=False,
    fit_templ=o_fit_templ,
    folder=o_fp,
):
    out_dict = {}
    for ind in sess_inds:
        az_ind = fit_templ.format(
            n_colors=n_colors,
            sess_ind=ind,
            period=period,
            ext="_az.nc",
        ...
        if os.path.isfile(az_fp):
            ...
            for k, v in data["diags"].items():
                if not v:
                    print("session {ind} has {k} warning".format(ind=ind, k=k))
...
nc_sweep_pattern = (
    "nc_{guess}(?P<decider>[a-zA-Z]+)_[0-9]+_{run_ind}_[0-9_\\-:.]+\\.pkl"
)


def load_nc_sweep(folder, run_ind, template=nc_sweep_pattern, **kwargs):
...
def load_circus_sweep(
    folder, swept_keys, store_keys=("cue1", "cue2"), template=circus_sweep_pattern
):
    fls = os.listdir(folder)
...
def load_many_sessions(session_nums, num_cols, time_period, time_bin, **kwargs):
    if time_period == "CUE2_ON_diode":
        k = dict(
            no_guess_model_names=cue_default_model_names,
    ...
    for sn in session_nums:
        session_dict[sn] = load_model_fits_templ(
            num_cols, sn, time_period, time_bin, **k
        )
    return session_dict


def load_model_fits_templ(
    num_cols,
    session_num,
    time_period,
    time_bin,
    template=model_folder_template,
    **kwargs,
):
...
def load_model_fits(
    folder,
    guess_model_names=cue_default_model_names,
    no_guess_model_names=wh_default_model_names,
    guess_model_keys=cue_default_model_keys,
    no_guess_model_keys=wh_default_model_keys,
    data_name="stan_data.pkl",
    load_guess=False,
    load_no_guess=True,
):
    load_names = ()
...
def load_bhv_data(
    ...
    if add_err:
        err = u.normalize_periodic_range(
            trl_dict["LABthetaTarget"] - trl_dict["LABthetaResp"]
        )
        trl_dict["err"] = err
...
def load_label_data(labelpath, unique_neurs, templ="([0-9a-zA-Z]+)\\.txt"):
    ...
    for fl in fls:
        m = re.match(templ, fl)
        if m is not None and (m.group(1) != "frontal" and m.group(1) != "posterior"):
...
def load_buschman_data(
    ...
    for fl in fls:
        ...
        if m is not None:
            ...
            spk_split, neurs = split_spks_bhv(
                chans,
                ids,
                ts,
            ...
            data_lab = load_label_data(os.path.join(folder, fl, label_sub), neurs)
            n_trls = spk_split.shape[0]
            d_dict = {}
            d_dict.update(trl_data)
            d_dict.update(
                (
                    ("spikeTimes", list(n for n in spk_split)),
