...
class TidyDataset(gio.Dataset):
    def __init__(self):
        super(TidyDataset, self).__init__()
        self.data = self.from_readfunc(swa.load_buschman_data, SAVE_DIR, max_files=np.inf,
            seconds=True, load_bhv_model=CODE_DIR+'/assignment_errors/bhv_model.pkl')

    def get_pop(self,tbeg=0, tend=0.5, twindow=0.5, tsetp=0.5,
        min_trials=40,tzf='CUE2_ON_diode', shuffle=False, repl_nan=False):

        um = self.data['is_one_sample_displayed'] == 0
        um = um.rs_and(self.data['Block']>1)
        um = um.rs_and(self.data['corr_prob']<2) # avoid nan trials
        data_single = self.data.mask(um)

        n = data_single.get_ntrls()

        self.pop, self.xs = data_single.get_populations(twindow, tbeg, tend, tstep,
            skl_axes=True, repl_nan=repl_nan, time_zero_field=tzf)   

        xs = xs[:int((tend-tbeg)/tstep)+1]

    def assemble_data(self, which_sess, tbeg=0, tend=0.5, twindow=0.5, tsetp=0.5, 
        num_bins=4, do_pca=True, pca_thrs=0.95, min_trials=40,tzf='CUE2_ON_diode',
        shuffle=False, repl_nan=False):
