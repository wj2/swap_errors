...
sys.path.append('C:/Users/mmall/Documents/github/jeffcode/')
...
for i in tqdm(range(num_pop)):
    ...
    perf = clf.score(binned_tst, np.repeat(labels_tst,np.sum(these_times)))
    
    err_perf.append(perf)


plt.errorbar(np.array(cv_perf).mean(1), err_test_perf, xerr=np.array(cv_perf).std(1), ls='none')
plt.scatter(np.array(cv_perf).mean(1), err_test_perf)
dicplt.square_axis()
