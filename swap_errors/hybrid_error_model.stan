...
model {
  ...
  for (n in 1:T) {
    ...
    // spatial errors
    lp_swp[1] = log_p_spa + (cue[n]*(normal_lpdf(y[n] | mu_u*C_l[n] + mu_d_l*C_u[n], sqrt(vars))) 
                        + (1-cue[n])*(normal_lpdf(y[n] | mu_l*C_u[n] + mu_d_u*C_l[n], sqrt(vars))));
    // cue errors
    lp_swp[2] = log_p_cue + ((1-cue[n])*(normal_lpdf(y[n] | mu_u*C_u[n] + mu_d_l*C_l[n], sqrt(vars))) 
