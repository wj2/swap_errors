...
class NRingModel:
    def __init__(
        ...
        n_units,
        tau=10,
        transfer_func=ring.relu_transfer,
        weight_func=ring.cosine_weightfunc,
        r_inhib=1,
        tf_params=(1,),
        r_beta=None,
        r_excit=1.5,
        i_ws=1,
        wf_params=default_wf_params,
        disorder_scale=1e-3,
        self_disorder=False,
        self_disorder_scale=None,
        **kwargs,
    ):
        if self_disorder_scale is None:
            self_disorder_scale = disorder_scale
        self.n_neurs = n_units
        self.n_rings = n_rings
        self.rings = {}
        for i in range(n_rings):
            self.rings[i] = ring.RingAttractor(
                n_units,
                tau,
                transfer_func,
                tf_params,
        ...
        self.thetas = self.rings[0].thetas

        self.rng = np.random.default_rng()
        self.connectivity = {}
        for i, j in it.combinations(range(n_rings), 2):
            self.connectivity[(i, j)] = (
                self.rng.normal(0, 1, size=(n_units, n_units))
                * disorder_scale
                / np.sqrt(n_units)
            )
            self.connectivity[(j, i)] = (
                self.rng.normal(0, 1, size=(n_units, n_units))
                * disorder_scale
                / np.sqrt(n_units)
            )
    ...
    def initialize_network(self, init_scale=0.01):
        self._curr_time = 0
        for r in self.rings.values():
            r.initialize_network(init_scale=init_scale)

    def iterate_step(self, drive, dt, dynamics_type="noiseless", dynamics_f=True):
        self._curr_time += dt
        for i, r in self.rings.items():
            ...
            inp_r = drive.get(i, 0.0)
            for j in range(self.n_rings):
                w_ij = self.connectivity.get(
                    (i, j), np.zeros((self.n_neurs, self.n_neurs))
                )
                inp_r += w_ij @ self.rings[j].get_output(dynamics_f=dynamics_f)
            r.iterate_step(inp_r, dt, dynamics)

    def state(self, dynamics_f=True):
        return {k: r.get_output(dynamics_f=dynamics_f) for k, r in self.rings.items()}
    ...
    def integrate_until(
        ...
        j = 0
        out = ring.get_steps_and_containers(
            self._curr_time, t_end, dt, keep, self.n_neurs, dict_keys=self.rings.keys()
        )
        steps_to_end, steps_to_keep, interval = out[:3]
        trace, focus, store_drive, time = out[3:]
        for i in range(steps_to_end):
            drive_out = {
                k: df(self._curr_time, self.state(), dt) for k, df in drive_dict.items()
            }
            ...
            if (i % interval) == 0 and j < steps_to_keep:
                ...
                time = add_dict_elements(time, self._curr_time, j, ne_dict=False)
                j = j + 1
        return trace, focus, store_drive, time

    def compute_bump_statistics(self):
        return self.rings[0].compute_bump_statistics()

    def compute_bump_statistics_empirical(self, **kwargs):
        return self.rings[0].compute_bump_statistics_empirical(**kwargs)

    def compute_pc(self, cue_mag):
        return self.rings[0].compute_pc(cue_mag)

    def compute_pc_empirical(self, cue_mag):
        return self.rings[0].compute_pc_empirical(cue_mag)

    def estimate_noise(self, **kwargs):
        return self.rings[0].estimate_noise(**kwargs)

    def estimate_pc_noise(self, cue_mag, **kwargs):
        return self.rings[0].estimate_pc_noise(cue_mag, **kwargs)
...
class FourRingModel:
    def __init__(
        ...
        **kwargs,
        ...
        self.ring_weights = dict(
            r_u=((w_lu, r_l), (w_tu, r_t), (w_du, r_d)),
            r_l=((w_ul, r_u), (w_tl, r_t), (w_dl, r_d)),
            r_t=((w_ut, r_u), (w_lt, r_l), (w_dt, r_d)),
            r_d=((w_ud, r_u), (w_ld, r_l), (w_td, r_t)),
        )
        # self.ring_weights = dict()

        out = self.make_input_connectivity(n_units, weight_scale=i_ws)
        w_iu, w_il, w_id, w_it = out
        self.input_weights = dict(r_u=(w_iu,), r_l=(w_il,), r_t=(w_it,), r_d=(w_id,))
        self.initialize_network()
    ...
    def make_input_connectivity(self, n_units, weight_scale=1):
        return self.make_ring_connectivity(
            n_units, weight_scale=n_units * weight_scale, conn_n=4
        )
...
def make_simple_drivers(
    ...
    drivers = {}
    for i in range(n_rings):
        drivers[i] = ring.step_drive_function_creator(
            thetas, color, col_width, stim_mag, stim_start, stim_start + stim_dur
        )
    return drivers
...
def make_drivers(
    ...
    d_u = ring.step_drive_function_creator(
        frm.thetas, c_u, col_width, stim_mag, stim_start, stim_start + stim_dur
    )
    d_l = ring.step_drive_function_creator(
        frm.thetas, c_l, col_width, stim_mag, stim_start, stim_start + stim_dur
    )
    ...
    if struct_cue:
        ...
        cue_pop_mask = None
    cue = ring.step_drive_function_creator(
        frm.thetas,
        0,
        2 * np.pi,
        gen_mag,
    ...
    cue_opp = ring.step_drive_function_creator(
        frm.thetas,
        0,
        2 * np.pi,
        cue_mag,
...
def simulate_trials(
    ...
    **driver_kwargs,
    ...
    for i in range(n_trls):
        ...
        out = frm.integrate_until(
            total_time, dt, drive_dict=drive_dict, dynamics_type=dynamics_type
        )
...
def plot_activity(activity, fwid=3, axs=None, use_all_max=True, set_min=0):
    if axs is None:
...
def plot_max_trace(time, activity, fwid=3, axs=None, vlines=None):
    if axs is None:
        f, axs = plt.subplots(
            2, 2, figsize=(fwid * 2, fwid * 2), sharex=True, sharey=False
        )
    axs_flat = np.reshape(axs, -1)
    for i, (k, act) in enumerate(activity.items()):
