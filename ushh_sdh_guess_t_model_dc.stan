...
parameters {
  ...
  real mu_u_raw[3, N, K];
  real mu_l_raw[3, N, K];
  real mu_d_u_raw[2, N, K];
  real mu_d_l_raw[2, N, K];
  
  vector[N] intercept_up;
  vector[N] intercept_down;
  matrix[3, N] i_up_raw;
  matrix[3, N] i_down_raw;
...
model {
  ...
  for (k in 1:K){
    ...
    for (i in 1:3) {
      ...
      mu_l_raw[i, :, k] ~ normal(0, 1);
      if (i < 3) {
	mu_d_u_raw[i, :, k] ~ normal(0, 1);
	mu_d_l_raw[i, :, k] ~ normal(0, 1);
      }
    }
  }
  ...
  // intercepts
  for (i in 1:3) {
    i_up_raw[i] ~ normal(0, 1);
    i_down_raw[i] ~ normal(0, 1);
  }
